...
/// Extension for mapping `HKQuantitySample` → `DietarySodiumRecordDto`.
extension HKQuantitySample {
    /// Converts this HealthKit quantity sample to a DietarySodiumRecordDto.
    ///
    /// - Returns: DietarySodiumRecordDto
    /// - Throws: `HealthConnectorError.invalidArgument` if quantity type mismatch
    func toDietarySodiumDto() throws -> DietarySodiumRecordDto {
...
/// Extension for mapping `DietarySodiumRecordDto` → `HKQuantitySample`.
extension DietarySodiumRecordDto {
    /// Converts this `DietarySodiumRecordDto` to its corresponding `HKQuantitySample`.
    ///
    ...
    func toHKQuantitySample() throws -> HKQuantitySample {
        let quantityType = try HKQuantityType.make(from: .dietarySodium)
