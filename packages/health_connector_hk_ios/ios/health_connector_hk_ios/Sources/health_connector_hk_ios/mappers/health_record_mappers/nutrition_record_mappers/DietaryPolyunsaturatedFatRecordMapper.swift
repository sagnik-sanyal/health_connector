...
/// Extension for mapping `HKQuantitySample` → `DietaryPolyunsaturatedFatRecordDto`.
extension HKQuantitySample {
    /// Converts this HealthKit quantity sample to a DietaryPolyunsaturatedFatRecordDto.
    ///
    /// - Returns: DietaryPolyunsaturatedFatRecordDto
    /// - Throws: `HealthConnectorError.invalidArgument` if quantity type mismatch
    func toDietaryPolyunsaturatedFatDto() throws -> DietaryPolyunsaturatedFatRecordDto {
        guard
            ...
            throw HealthConnectorError.invalidArgument(
                message:
                "Expected dietary fat polyunsaturated quantity type, got \(quantityType.identifier)",
                context: [
...
/// Extension for mapping `DietaryPolyunsaturatedFatRecordDto` → `HKQuantitySample`.
extension DietaryPolyunsaturatedFatRecordDto {
    /// Converts this `DietaryPolyunsaturatedFatRecordDto` to its corresponding `HKQuantitySample`.
    ///
    ...
    func toHKQuantitySample() throws -> HKQuantitySample {
        let quantityType = try HKQuantityType.make(from: .dietaryFatPolyunsaturated)
