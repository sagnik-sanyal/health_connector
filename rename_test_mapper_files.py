#!/usr/bin/env python3
"""
Script to rename test files from *_mappers_test.dart to *_mapper_test.dart
...
def find_files_to_rename(base_dir: Path) -> list[tuple[Path, Path]]:
    """
    Find all test files ending with _mappers_test.dart and their new names.
    ...
    for dart_file in base_dir.rglob("*_mappers_test.dart"):
        new_name = dart_file.name.replace("_mappers_test.dart", "_mapper_test.dart")
...
def update_imports_in_file(file_path: Path, old_filename: str, new_filename: str) -> int:
    ...
    try:
        ...
        old_pattern = old_filename.replace("_mappers_test.dart", "_mappers_test")
        new_pattern = new_filename.replace("_mapper_test.dart", "_mapper_test")
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        print("Usage: python3 rename_test_mapper_files.py <directory>")
        return
    ...
    print(f"Finding test files to rename (*_mappers_test.dart -> *_mapper_test.dart)...\n")

    # Find all files to rename
    renames = find_files_to_rename(base_dir)

    if not renames:
        print("No files found matching pattern *_mappers_test.dart")
        return
    ...
    print(f"Found {len(renames)} test files to rename")
    ...
    print(f"\n{'=' * 60}")
    print(f"Summary:")
    print(f"  Test files renamed: {renamed_count}")
