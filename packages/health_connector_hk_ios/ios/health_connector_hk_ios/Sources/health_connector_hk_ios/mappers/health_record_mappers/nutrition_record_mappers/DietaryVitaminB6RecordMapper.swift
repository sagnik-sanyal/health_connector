...
/// Extension for mapping `HKQuantitySample` → `DietaryVitaminB6RecordDto`.
extension HKQuantitySample {
    /// Converts this HealthKit quantity sample to a DietaryVitaminB6RecordDto.
    ///
    /// - Returns: DietaryVitaminB6RecordDto
    /// - Throws: `HealthConnectorError.invalidArgument` if quantity type mismatch
    func toDietaryVitaminB6Dto() throws -> DietaryVitaminB6RecordDto {
        guard quantityType.identifier == HKQuantityTypeIdentifier.dietaryVitaminB6.rawValue
        else {
            throw HealthConnectorError.invalidArgument(
                message:
                "Expected dietary vitamin B6 quantity type, got \(quantityType.identifier)",
                context: [
...
/// Extension for mapping `DietaryVitaminB6RecordDto` → `HKQuantitySample`.
extension DietaryVitaminB6RecordDto {
    /// Converts this `DietaryVitaminB6RecordDto` to its corresponding `HKQuantitySample`.
    ///
