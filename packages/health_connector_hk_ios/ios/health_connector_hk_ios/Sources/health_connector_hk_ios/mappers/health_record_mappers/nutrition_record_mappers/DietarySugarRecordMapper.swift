...
/// Extension for mapping `HKQuantitySample` → `DietarySugarRecordDto`.
extension HKQuantitySample {
    /// Converts this HealthKit quantity sample to a DietarySugarRecordDto.
    ///
    /// - Returns: DietarySugarRecordDto
    /// - Throws: `HealthConnectorError.invalidArgument` if quantity type mismatch
    func toDietarySugarDto() throws -> DietarySugarRecordDto {
...
/// Extension for mapping `DietarySugarRecordDto` → `HKQuantitySample`.
extension DietarySugarRecordDto {
    /// Converts this `DietarySugarRecordDto` to its corresponding `HKQuantitySample`.
    ///
