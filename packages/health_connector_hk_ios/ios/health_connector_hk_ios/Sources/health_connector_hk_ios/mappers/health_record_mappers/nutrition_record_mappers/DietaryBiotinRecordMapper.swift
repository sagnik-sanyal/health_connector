...
/// Extension for mapping `HKQuantitySample` → `DietaryBiotinRecordDto`.
extension HKQuantitySample {
    /// Converts this HealthKit quantity sample to a DietaryBiotinRecordDto.
    ///
    /// - Returns: DietaryBiotinRecordDto
    /// - Throws: `HealthConnectorError.invalidArgument` if quantity type mismatch
    func toDietaryBiotinDto() throws -> DietaryBiotinRecordDto {
...
/// Extension for mapping `DietaryBiotinRecordDto` → `HKQuantitySample`.
extension DietaryBiotinRecordDto {
    /// Converts this `DietaryBiotinRecordDto` to its corresponding `HKQuantitySample`.
    ///
