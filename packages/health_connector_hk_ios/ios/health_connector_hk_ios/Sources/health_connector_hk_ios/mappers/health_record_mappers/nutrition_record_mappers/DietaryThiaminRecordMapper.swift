...
/// Extension for mapping `HKQuantitySample` → `DietaryThiaminRecordDto`.
extension HKQuantitySample {
    /// Converts this HealthKit quantity sample to a DietaryThiaminRecordDto.
    ///
    /// - Returns: DietaryThiaminRecordDto
    /// - Throws: `HealthConnectorError.invalidArgument` if quantity type mismatch
    func toDietaryThiaminDto() throws -> DietaryThiaminRecordDto {
...
/// Extension for mapping `DietaryThiaminRecordDto` → `HKQuantitySample`.
extension DietaryThiaminRecordDto {
    /// Converts this `DietaryThiaminRecordDto` to its corresponding `HKQuantitySample`.
    ///
    ...
    func toHKQuantitySample() throws -> HKQuantitySample {
        let quantityType = try HKQuantityType.make(from: .dietaryThiamin)
