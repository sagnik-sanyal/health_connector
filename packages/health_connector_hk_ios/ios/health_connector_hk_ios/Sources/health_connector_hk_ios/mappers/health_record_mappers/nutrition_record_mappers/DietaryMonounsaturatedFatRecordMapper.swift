...
/// Extension for mapping `HKQuantitySample` → `DietaryMonounsaturatedFatRecordDto`.
...
/// Extension for mapping `DietaryMonounsaturatedFatRecordDto` → `HKQuantitySample`.
extension DietaryMonounsaturatedFatRecordDto {
    /// Converts this `DietaryMonounsaturatedFatRecordDto` to its corresponding `HKQuantitySample`.
    ///
