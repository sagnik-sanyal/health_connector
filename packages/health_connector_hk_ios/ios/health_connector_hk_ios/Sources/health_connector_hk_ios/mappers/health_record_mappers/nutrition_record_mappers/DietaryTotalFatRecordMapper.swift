...
/// Extension for mapping `HKQuantitySample` → `DietaryTotalFatRecordDto`.
...
/// Extension for mapping `DietaryTotalFatRecordDto` → `HKQuantitySample`.
extension DietaryTotalFatRecordDto {
    /// Converts this `DietaryTotalFatRecordDto` to its corresponding `HKQuantitySample`.
    ///
