...
/// Extension for mapping `HKQuantitySample` → `DietaryManganeseRecordDto`.
extension HKQuantitySample {
    /// Converts this HealthKit quantity sample to a DietaryManganeseRecordDto.
    ///
    /// - Returns: DietaryManganeseRecordDto
    /// - Throws: `HealthConnectorError.invalidArgument` if quantity type mismatch
    func toDietaryManganeseDto() throws -> DietaryManganeseRecordDto {
...
/// Extension for mapping `DietaryManganeseRecordDto` → `HKQuantitySample`.
extension DietaryManganeseRecordDto {
    /// Converts this `DietaryManganeseRecordDto` to its corresponding `HKQuantitySample`.
    ///
