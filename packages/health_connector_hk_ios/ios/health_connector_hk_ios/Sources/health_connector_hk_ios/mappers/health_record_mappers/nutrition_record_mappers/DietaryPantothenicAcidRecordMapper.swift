...
/// Extension for mapping `HKQuantitySample` → `DietaryPantothenicAcidRecordDto`.
extension HKQuantitySample {
    /// Converts this HealthKit quantity sample to a DietaryPantothenicAcidRecordDto.
    ///
    /// - Returns: DietaryPantothenicAcidRecordDto
    /// - Throws: `HealthConnectorError.invalidArgument` if quantity type mismatch
    func toDietaryPantothenicAcidDto() throws -> DietaryPantothenicAcidRecordDto {
        guard
            quantityType.identifier == HKQuantityTypeIdentifier.dietaryPantothenicAcid.rawValue
        else {
            throw HealthConnectorError.invalidArgument(
                message:
                "Expected dietary pantothenic acid quantity type, got \(quantityType.identifier)",
                context: [
...
/// Extension for mapping `DietaryPantothenicAcidRecordDto` → `HKQuantitySample`.
extension DietaryPantothenicAcidRecordDto {
    /// Converts this `DietaryPantothenicAcidRecordDto` to its corresponding `HKQuantitySample`.
    ///
    ...
    func toHKQuantitySample() throws -> HKQuantitySample {
        let quantityType = try HKQuantityType.make(from: .dietaryPantothenicAcid)
