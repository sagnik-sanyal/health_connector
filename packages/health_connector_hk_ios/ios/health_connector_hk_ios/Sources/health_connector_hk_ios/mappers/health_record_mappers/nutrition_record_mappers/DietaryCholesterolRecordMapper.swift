...
/// Extension for mapping `HKQuantitySample` → `DietaryCholesterolRecordDto`.
extension HKQuantitySample {
    /// Converts this HealthKit quantity sample to a DietaryCholesterolRecordDto.
    ///
    /// - Returns: DietaryCholesterolRecordDto
    /// - Throws: `HealthConnectorError.invalidArgument` if quantity type mismatch
    func toDietaryCholesterolDto() throws -> DietaryCholesterolRecordDto {
        guard quantityType.identifier == HKQuantityTypeIdentifier.dietaryCholesterol.rawValue
        else {
            throw HealthConnectorError.invalidArgument(
                message:
                "Expected dietary cholesterol quantity type, got \(quantityType.identifier)",
                context: [
...
/// Extension for mapping `DietaryCholesterolRecordDto` → `HKQuantitySample`.
extension DietaryCholesterolRecordDto {
    /// Converts this `DietaryCholesterolRecordDto` to its corresponding `HKQuantitySample`.
    ///
