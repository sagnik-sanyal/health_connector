...
@DisplayName("BasalMetabolicRateRecordMapper")
class BasalMetabolicRateRecordMapperTest {

    private companion object {
        const val TEST_ID = "bmr-id"
        const val TEST_TIME = 1609459200000L
        const val TEST_KILOCALORIES_PER_DAY = 1800.0
        val TEST_ZONE_OFFSET: ZoneOffset = ZoneOffset.ofHours(1)
    }

    @Test
    @DisplayName("GIVEN BasalMetabolicRateRecord → WHEN toDto → THEN converts correctly")
    fun whenRecordToDto_thenConvertsCorrectly() {
        // Given
        val record = BasalMetabolicRateRecord(
            basalMetabolicRate = Power.kilocaloriesPerDay(TEST_KILOCALORIES_PER_DAY),
        ...
        result.kilocaloriesPerDay shouldBe TEST_KILOCALORIES_PER_DAY
    }

    @Test
    @DisplayName(
        "GIVEN BasalMetabolicRateRecordDto → WHEN toHealthConnect → THEN creates correctly",
    )
    fun whenDtoToHealthConnect_thenCreatesCorrectly() {
        // Given
        val dto = BasalMetabolicRateRecordDto(
            id = TEST_ID,
            time = TEST_TIME,
            zoneOffsetSeconds = TEST_ZONE_OFFSET.totalSeconds.toLong(),
            kilocaloriesPerDay = TEST_KILOCALORIES_PER_DAY,
        ...
        // Then
        result.basalMetabolicRate.inKilocaloriesPerDay shouldBe TEST_KILOCALORIES_PER_DAY
