...
@DisplayName("BodyWaterMassRecordMapper")
class BodyWaterMassRecordMapperTest {

    private companion object {
        const val TEST_ID = "body-water-id"
        const val TEST_TIME = 1609459200000L
        const val TEST_MASS_KG = 45.0
        val TEST_ZONE_OFFSET: ZoneOffset = ZoneOffset.ofHours(1)
    }

    @Test
    @DisplayName("GIVEN BodyWaterMassRecord → WHEN toDto → THEN converts correctly")
    fun whenRecordToDto_thenConvertsCorrectly() {
        // Given
        val record = BodyWaterMassRecord(
    ...
    @Test
    @DisplayName("GIVEN BodyWaterMassRecordDto → WHEN toHealthConnect → THEN creates correctly")
    fun whenDtoToHealthConnect_thenCreatesCorrectly() {
        // Given
        val dto = BodyWaterMassRecordDto(
            id = TEST_ID,
            time = TEST_TIME,
