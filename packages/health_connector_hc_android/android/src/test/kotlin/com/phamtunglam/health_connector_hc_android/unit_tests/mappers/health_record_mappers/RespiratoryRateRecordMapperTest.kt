...
@DisplayName("RespiratoryRateRecordMapper")
class RespiratoryRateRecordMapperTest {

    private companion object {
        const val TEST_ID = "respiratory-rate-id"
        const val TEST_TIME = 1609459200000L
        const val TEST_RATE = 15.0
        val TEST_ZONE_OFFSET: ZoneOffset = ZoneOffset.ofHours(1)
    }

    @Test
    @DisplayName("GIVEN RespiratoryRateRecord → WHEN toDto → THEN converts correctly")
    ...
    fun whenDtoToHealthConnect_thenCreatesCorrectly() {
        ...
        // Then
        result.rate shouldBe TEST_RATE
