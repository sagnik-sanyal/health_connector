...
/// Extension for mapping `HKQuantitySample` → `DietaryCalciumRecordDto`.
extension HKQuantitySample {
    /// Converts this HealthKit quantity sample to a DietaryCalciumRecordDto.
    ///
    /// - Returns: DietaryCalciumRecordDto
    /// - Throws: `HealthConnectorError.invalidArgument` if quantity type mismatch
    func toDietaryCalciumDto() throws -> DietaryCalciumRecordDto {
...
/// Extension for mapping `DietaryCalciumRecordDto` → `HKQuantitySample`.
extension DietaryCalciumRecordDto {
    /// Converts this `DietaryCalciumRecordDto` to its corresponding `HKQuantitySample`.
    ///
    ...
    func toHKQuantitySample() throws -> HKQuantitySample {
        let quantityType = try HKQuantityType.make(from: .dietaryCalcium)
