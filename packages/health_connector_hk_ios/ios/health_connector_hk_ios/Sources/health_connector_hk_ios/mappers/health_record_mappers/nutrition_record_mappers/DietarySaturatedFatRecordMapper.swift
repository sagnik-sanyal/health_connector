...
/// Extension for mapping `HKQuantitySample` → `DietarySaturatedFatRecordDto`.
extension HKQuantitySample {
    /// Converts this HealthKit quantity sample to a DietarySaturatedFatRecordDto.
    ///
    /// - Returns: DietarySaturatedFatRecordDto
    /// - Throws: `HealthConnectorError.invalidArgument` if quantity type mismatch
    func toDietarySaturatedFatDto() throws -> DietarySaturatedFatRecordDto {
        guard quantityType.identifier == HKQuantityTypeIdentifier.dietaryFatSaturated.rawValue
        else {
            throw HealthConnectorError.invalidArgument(
                message:
                "Expected dietary fat saturated quantity type, got \(quantityType.identifier)",
                context: [
...
/// Extension for mapping `DietarySaturatedFatRecordDto` → `HKQuantitySample`.
extension DietarySaturatedFatRecordDto {
    /// Converts this `DietarySaturatedFatRecordDto` to its corresponding `HKQuantitySample`.
    ///
