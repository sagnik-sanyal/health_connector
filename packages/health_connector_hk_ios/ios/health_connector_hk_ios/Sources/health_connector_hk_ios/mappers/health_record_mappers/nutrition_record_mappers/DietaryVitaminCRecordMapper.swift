...
/// Extension for mapping `HKQuantitySample` → `DietaryVitaminCRecordDto`.
extension HKQuantitySample {
    /// Converts this HealthKit quantity sample to a DietaryVitaminCRecordDto.
    ///
    /// - Returns: DietaryVitaminCRecordDto
    /// - Throws: `HealthConnectorError.invalidArgument` if quantity type mismatch
    func toDietaryVitaminCDto() throws -> DietaryVitaminCRecordDto {
...
/// Extension for mapping `DietaryVitaminCRecordDto` → `HKQuantitySample`.
extension DietaryVitaminCRecordDto {
    /// Converts this `DietaryVitaminCRecordDto` to its corresponding `HKQuantitySample`.
    ///
    ...
    func toHKQuantitySample() throws -> HKQuantitySample {
        let quantityType = try HKQuantityType.make(from: .dietaryVitaminC)
