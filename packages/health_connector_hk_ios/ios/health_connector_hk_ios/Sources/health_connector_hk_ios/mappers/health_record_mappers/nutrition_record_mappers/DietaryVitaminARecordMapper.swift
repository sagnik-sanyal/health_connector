...
/// Extension for mapping `HKQuantitySample` → `DietaryVitaminARecordDto`.
extension HKQuantitySample {
    /// Converts this HealthKit quantity sample to a DietaryVitaminARecordDto.
    ///
    /// - Returns: DietaryVitaminARecordDto
    /// - Throws: `HealthConnectorError.invalidArgument` if quantity type mismatch
    func toDietaryVitaminADto() throws -> DietaryVitaminARecordDto {
...
/// Extension for mapping `DietaryVitaminARecordDto` → `HKQuantitySample`.
extension DietaryVitaminARecordDto {
    /// Converts this `DietaryVitaminARecordDto` to its corresponding `HKQuantitySample`.
    ///
    ...
    func toHKQuantitySample() throws -> HKQuantitySample {
        let quantityType = try HKQuantityType.make(from: .dietaryVitaminA)
