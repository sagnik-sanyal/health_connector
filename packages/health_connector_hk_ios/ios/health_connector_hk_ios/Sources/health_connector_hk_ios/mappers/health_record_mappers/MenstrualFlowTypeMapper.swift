...
extension MenstrualFlowTypeDto {
    /// Converts `MenstrualFlowTypeDto` to HealthKit menstrual flow value.
    ///
...
extension Int {
    /// Converts a HealthKit menstrual flow value to `MenstrualFlowTypeDto`.
    ///
    /// Handles both iOS 17 and iOS 18+ enum types, mapping `.none` to `.unknown`.
    func toMenstrualFlowTypeDto() -> MenstrualFlowTypeDto {
        if #available(iOS 18.0, *) {
